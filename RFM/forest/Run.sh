...
if [[ "$choice" == "Train" && "$data" == "s" ]]; then
    ...
    spark-submit --class com.forest.TestModel --master spark://$master.cs.colostate.edu:$port target/forest-1.0-SNAPSHOT.jar hdfs://$master.cs.colostate.edu:$hport/CS435-Term-Project/Data_sample/* Model
elif [[ "$choice" == "Test" && "$data" == "r" ]]; then
    spark-submit --class com.forest.TestModel --master spark://$master.cs.colostate.edu:$port target/forest-1.0-SNAPSHOT.jar hdfs://$master.cs.colostate.edu:$hport/CS435-Term-Project/Data/* Model
fi